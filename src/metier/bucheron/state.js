// Gestion centralisee de l'etat du metier bucheron (niveau, xp, palier suivant)
import { bucheronDefinition } from "./config.js";
...
/**
 * Retourne l'etat metier bucheron du joueur en garantissant
 * les proprietes level / xp / xpNext.
 */
export function ensureBucheronState(player) {
  if (!player) return null;
  if (!player.metiers) {
    player.metiers = {};
  }
  if (!player.metiers.bucheron) {
    player.metiers.bucheron = {
      level: DEFAULT_LEVEL,
      xp: 0,
  ...
  const state = player.metiers.bucheron;
  // Securise les champs
...
/**
 * Ajoute de l'XP au metier bucheron et gere les montees de niveau.
 * Retourne le nouvel etat et si un niveau a ete gagne.
 */
export function addBucheronXp(player, amount) {
...
// Expose la définition statique au même endroit pour simplifier les imports
export { bucheronDefinition };
