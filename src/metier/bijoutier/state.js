import { bijoutierDefinition } from "./config.js";
...
export function ensureBijoutierState(player) {
  if (!player) return null;
  if (!player.metiers) player.metiers = {};
  if (!player.metiers.bijoutier) {
    player.metiers.bijoutier = {
      level: DEFAULT_LEVEL,
      xp: 0,
      xpNext: computeXpNext(DEFAULT_LEVEL),
    };
  }
  const state = player.metiers.bijoutier;
...
export function addBijoutierXp(player, amount) {
