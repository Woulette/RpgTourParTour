...
// IA du Liburion : lanceur en ligne (mid-range).
// - cherche à se placer en ligne à 2-4 cases
// - lance Éclat liburien jusqu'à 2 fois si possible
export function runTurn(scene, state, monster, player, map, groundLayer, onComplete) {
  ...
  const tryCast = () => {
    ...
    const canHit = canCastSpellOnTile(scene, monster, eclat, px, py, map);
  ...
  const fleeAfterAttack = () => {
    ...
    // Petit dèlai apräs les sorts pour lisibilitè
    delay(scene, 220, () => {
  ...
  const castSequence = (count) => {
    ...
    if (scene.time?.delayedCall) {
      scene.time.delayedCall(450, () => castSequence(count - 1));
    } else {
      castSequence(count - 1);
    }
  };

  // Déjà à portée -> cast direct
  if (eclat && canCastSpellOnTile(scene, monster, eclat, px, py, map)) {
    delay(scene, 220, () => castSequence(2));
    return;
  }

  // Mouvement : on cherche une case "post de tir" idéale (alignée + portée 2..4),
  // sinon on avance vers une case qui rapproche d'une position de tir.
  ...
  // On évalue quelques cibles proches autour du joueur, puis fallback sur un objectif "proche du tir".
  const candidates = [];
  for (let d = 2; d <= 4; d += 1) {
  ...
  // 1) Si une case de tir est atteignable, on prend la plus proche en path.
  ...
  // 2) Sinon, on cherche une "bonne" case à atteindre plus tard :
  // alignée si possible, distance proche de 3, et path le plus court.
  if (!bestPath) {
    const scan = [];
    for (let r = 1; r <= 6; r += 1) {
  ...
  moveMonsterAlongPath(scene, monster, map, groundLayer, pathTiles, () => {
    ...
    // Après déplacement, on tente de caster si possible.
    delay(scene, 520, () => {
