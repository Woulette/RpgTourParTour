...
export function runTurn(scene, state, monster, player, map, groundLayer, onComplete) {
  ...
  // Tente d'abord le sort à distance (lisibilité -> delay)
  if (dist >= 2 && dist <= 4 && tryCast(ranged)) {
    delay(scene, 260, finish);
    return;
  }

  // Si au contact, spam griffure
  if (dist === 1) {
  ...
  // Déplacement vers contact puis attaque
  if (pmRestants <= 0) {
    finish();
    return;
  }
  ...
  if (pathTiles.length === 0) {
    // Si on ne peut pas approcher, tente quand même un projectile si possible
  ...
  moveMonsterAlongPath(scene, monster, map, groundLayer, pathTiles, () => {
    ...
    if (pathTiles.length > 0) {
      showFloatingTextOverEntity(scene, monster, `${pathTiles.length}`, { color: "#22c55e" });
    }
    delay(scene, 520, () => {
      // après move: projectile si possible sinon griffure
