...
export function runTurn(scene, state, monster, player, map, groundLayer, onComplete) {
  ...
  // Poison prioritaire si possible (et pas déjà actif)
  ...
  // Se rapprocher puis essayer ronce
  if (pmRestants <= 0) {
    finish();
    return;
  }
  ...
  if (pathTiles.length === 0) {
    // Si bloqué, tente un poison (même si déjà actif, il ne passera pas si cooldown)
  ...
  moveMonsterAlongPath(scene, monster, map, groundLayer, pathTiles, () => {
    ...
    delay(scene, 520, () => {
      // Après move : poison si possible sinon ronce si contact
