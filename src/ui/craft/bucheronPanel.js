import { ensureBucheronState, addBucheronXp } from "../../metier/bucheron/state.js";
import { bucheronRecipes } from "../../metier/bucheron/recipes.js";
...
function ensurePanelElements() {
        ...
        <section class="craft-left craft-left-alchimiste">
          <div class="alchimiste-xp-card">
            <div class="alchimiste-xp-top">
              ...
              <span class="alchimiste-xp-level">Niv. <span id="bucheron-xp-level">1</span></span>
            </div>
            <div class="craft-xp-bar">
              <div class="craft-xp-bar-fill" id="bucheron-xp-fill"></div>
              <div class="craft-xp-bar-label" id="bucheron-xp-label">XP 0 / 100</div>
            </div>
          </div>
          <ul class="craft-recipes" id="bucheron-recipes"></ul>
        </section>
        <section class="craft-center craft-center-alchimiste">
          <div class="alchimiste-top">
            <div class="alchimiste-slots-row" id="bucheron-slots"></div>
            <button type="button" class="craft-btn" id="bucheron-craft-btn">Preparer</button>
          </div>
          <div class="craft-result empty" id="bucheron-result">
            <span>Aucun craft realise.</span>
          </div>
        </section>
        <section class="craft-inventory craft-inventory-alchimiste">
          <div class="craft-inventory-header">
            <div class="craft-inventory-filters" id="bucheron-inventory-filters">
          ...
          <div class="craft-inventory-grid" id="bucheron-inventory"></div>
        </section>
      </div>
    </div>
  `;
  ...
  closeBtn.addEventListener("click", () => closeBucheronCraftPanel());
...
function renderXpHeader(player) {
  ...
  const levelEl = panelEl.querySelector("#bucheron-xp-level");
  const fillEl = panelEl.querySelector("#bucheron-xp-fill");
  const labelEl = panelEl.querySelector("#bucheron-xp-label");
...
function renderSlots(recipe, player) {
  const slotsEl = panelEl.querySelector("#bucheron-slots");
  if (!slotsEl) return;
  slotsEl.innerHTML = "";
...
function renderResult(player) {
  const resultEl = panelEl.querySelector("#bucheron-result");
  if (!resultEl) return;
  resultEl.innerHTML = "";
...
function renderInventory(player) {
  const grid = panelEl.querySelector("#bucheron-inventory");
  if (!grid) return;
  grid.innerHTML = "";
  ...
  const filterBtn = panelEl.querySelector(
    "#bucheron-inventory-filters button.active"
  );
...
function renderRecipes(player, selectedIdRef) {
  const list = panelEl.querySelector("#bucheron-recipes");
  if (!list) return null;
  list.innerHTML = "";
  ...
  let currentSelected = selectedIdRef?.value || (bucheronRecipes[0] && bucheronRecipes[0].id);

  bucheronRecipes.forEach((recipe) => {
...
function updateCraftButton(recipe, player) {
  const btn = panelEl.querySelector("#bucheron-craft-btn");
...
export function openBucheronCraftPanel(scene, player) {
  ensurePanelElements();
  if (!panelEl) return;
  const selectedRef = { value: bucheronRecipes[0]?.id };
  const selected = renderRecipes(player, selectedRef);
  const getActiveRecipe = () =>
    bucheronRecipes.find((r) => r.id === selectedRef.value) ||
    bucheronRecipes.find((r) => r.id === selected) ||
    bucheronRecipes[0];
  ...
  if (btn) {
    btn.onclick = () => {
      ...
      const { state } = addBucheronXp(player, activeRecipe.xpGain ?? 0);
      if (state) {
...
export function closeBucheronCraftPanel() {
  if (!panelEl) return;
