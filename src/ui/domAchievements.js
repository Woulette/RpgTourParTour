...
export function openAchievementsTo({ packId, achievementId } = {}) {
  if (!achievementsUiPlayer || !openAchievementsPanel || !renderAchievementsPanel) return false;
...
function updateHonor() {
  ...
  function renderDetail(entry) {
    ...
    reqs.forEach((req) => {
      ...
      if (req.type === "quest_completed") {
        const done = isQuestCompleted(player, req.questId);
        li.textContent = `${done ? "[x]" : "[ ]"} ${questTitle(req.questId)}`;
      } else if (req.type === "quest_stage_index_at_least") {
        const stageIndex = player?.quests?.[req.questId]?.stageIndex ?? 0;
        ...
        const progress = getAchievementProgress(player, req.achievementId);
  ...
  function renderPackDetail(packEntry) {
    ...
    if (claimBtn) {
      claimBtn.onclick = () => {
        if (!selected) return;
        const result = claimAchievement(player, selected.def.id);
  ...
  function render() {
    ...
    const packEntriesAll = achievementPackDefs.map((p) => packProgress(player, p));
