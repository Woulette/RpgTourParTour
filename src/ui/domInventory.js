  ...
  function renderInventory() {
    ...
    if (true) {
      ...
      for (let v = 0; v < inv.size; v += 1) {
        ...
        const realIndex = entry ? entry.realIndex : null;
        const slotData = entry ? entry.slotData : null;

        const slot = document.createElement("button");
        slot.type = "button";
        slot.className = "inventory-slot";
        slot.dataset.index = String(v);
        if (typeof realIndex === "number") slot.dataset.realIndex = String(realIndex);
        ...
        slot.addEventListener("click", () => {
          ...
          showItemDetailsById(liveSlotData.itemId);
        });
