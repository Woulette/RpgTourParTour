import { ensureBijoutierState, addBijoutierXp } from "../../bijoutier/state.js";
...
function ensurePanelElements() {
  ...
  ensureLink("craft-bijoutier-css", "assets/css/craft-bijoutier.css");

  panelEl = document.createElement("div");
  panelEl.id = "bijoutier-craft-panel";
  panelEl.className = "craft-panel bijoutier";
  panelEl.innerHTML = `
    <div class="craft-panel-inner">
      <header class="craft-panel-header">
        <h3>Table de Bijoutier</h3>
      ...
      <div class="craft-body">
        <section class="craft-left">
          <div class="craft-xp-row">
            <div class="craft-xp-bar">
              <div class="craft-xp-bar-fill" id="bijoutier-xp-fill"></div>
              <div class="craft-xp-bar-label" id="bijoutier-xp-label">XP 0 / 100</div>
            </div>
            <div class="craft-xp-level">Niv. <span id="bijoutier-xp-level">1</span></div>
          </div>
          <div class="craft-recipe-filters">
            <input
              ...
              id="bijoutier-filter-level"
              placeholder="Niv. max"
            />
            <input
              type="text"
              class="craft-filter-search"
              id="bijoutier-filter-search"
              placeholder="Rechercher..."
            />
            <select class="craft-filter-type" id="bijoutier-filter-type">
              <option value="all">Tout</option>
              <option value="anneau">Anneau</option>
              <option value="amulette">Amulette</option>
            </select>
          </div>
          <ul class="craft-recipes" id="bijoutier-recipes"></ul>
        </section>
        <section class="craft-center">
          <div class="craft-slots" id="bijoutier-slots"></div>
          <div class="craft-actions">
            <button type="button" class="craft-btn" id="bijoutier-craft-btn">Crafter</button>
            <div class="craft-result empty" id="bijoutier-result">
        ...
        <section class="craft-inventory">
          <div class="craft-inventory-header">
            <div class="craft-inventory-filters" id="bijoutier-inventory-filters">
          ...
          <div class="craft-inventory-grid" id="bijoutier-inventory"></div>
        </section>
      </div>
    </div>
  `;
  ...
  closeBtn.addEventListener("click", () => closeBijoutierCraftPanel());

  return panelEl;
}

function renderInventory(player) {
  const grid = panelEl.querySelector("#bijoutier-inventory");
  if (!grid) return;
  grid.innerHTML = "";
...
function renderSlots(recipe, player) {
  const slotsEl = panelEl.querySelector("#bijoutier-slots");
  if (!slotsEl) return;
  slotsEl.innerHTML = "";
...
function renderResult(player) {
  const resultEl = panelEl.querySelector("#bijoutier-result");
  if (!resultEl) return;
  resultEl.innerHTML = "";
...
function getRecipeFilters() {
  const levelInput = panelEl.querySelector("#bijoutier-filter-level");
  const searchInput = panelEl.querySelector("#bijoutier-filter-search");
  const typeSelect = panelEl.querySelector("#bijoutier-filter-type");
...
function getFilteredRecipes() {
  ...
  const sortedRecipes = bijoutierRecipes.slice().sort((a, b) => {
...
function renderRecipes(player, selectedIdRef, filteredRecipes) {
  const list = panelEl.querySelector("#bijoutier-recipes");
  if (!list) return;
  list.innerHTML = "";

  const inventory = player?.inventory;
  const state = ensureBijoutierState(player);
...
function renderXpHeader(player) {
  const state = ensureBijoutierState(player);
  if (!state) return;
  const fill = panelEl.querySelector("#bijoutier-xp-fill");
  const label = panelEl.querySelector("#bijoutier-xp-label");
  const lvl = panelEl.querySelector("#bijoutier-xp-level");
  const percent =
...
export function openBijoutierCraftPanel(scene, player) {
  ensurePanelElements();
  if (!panelEl) return;
  ...
  if (!craftUnsub) {
    craftUnsub = onStoreEvent("craft:completed", (payload) => {
      if (!payload || payload.metierId !== "bijoutier") return;
...
export function closeBijoutierCraftPanel() {
  if (!panelEl) return;
