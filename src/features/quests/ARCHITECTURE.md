...
- `src/features/quests/catalog.js` : définitions immuables des quêtes (`QuestDef`).
- `src/features/quests/defs/` : fichiers de définitions par quête (data pure).
- `src/features/quests/state.js` : état par joueur (`QuestState`) + progression (accept/advance/complete).
- `src/features/quests/runtime/` : logique générique pour PNJ (`!/?`) et objectifs.

## Marqueurs `!` / `?`

Calcul centralisé dans `src/features/quests/runtime/npcStatus.js` :
...
Gérés dans `src/features/quests/runtime/objectives.js` :
