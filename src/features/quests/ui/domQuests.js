import { getAllQuestStates, QUEST_STATES } from "../index.js";
import { on as onStoreEvent, getPlayer } from "../../../state/store.js";
import {
  countItemInInventory,
  getCraftedCount,
  hasAppliedParchment,
} from "../runtime/objectives.js";
...
export function initDomQuests(player) {
    ...
    if (detailProgressEl) {
      ...
      if (objective && objective.type === "kill_monster") {
        ...
        const current = isParchmentStep
          ? hasAppliedParchment(currentPlayer, state)
            ? 1
            : 0
        ...
        const current = Math.min(
          required,
          countItemInInventory(currentPlayer, objective.itemId)
        );
        detailProgressEl.textContent = `${objective.label}: ${current}/${required}`;
        if (detailProgressBarFillEl) {
          const percent = (current / required) * 100;
        ...
        const parts = items
          ...
          .map((it) => {
            ...
            const current = Math.min(
              required,
              countItemInInventory(currentPlayer, it.itemId)
            );
        ...
        const current = Math.min(
          ...
          items.reduce((acc, it) => {
            ...
            const cur = Math.min(
              req,
              getCraftedCount(currentPlayer, state, it.itemId)
            );
            return acc + cur;
          }, 0)
        );
  ...
  const renderLists = () => {
    ...
    const all = getAllQuestStates(currentPlayer);
